...
// If an encryption key is configured, pass it to the FileStore so session files
// are actually encrypted. Previously the key was present in config but never
// forwarded to FileStore which caused logs to incorrectly claim encryption.
if (config.FILESTORE_ENCRYPTION_KEY) {
  sessionStoreOptions.secret = config.FILESTORE_ENCRYPTION_KEY;
}

const sessionStore = new FileStore(sessionStoreOptions);
  ...
  try {
    ...
    sessionStore.get(cleanSessionId, (err, sessionObj) => {
      ...
      callback(null, {
        ...
        session: {
          turnstileToken: sessionObj.turnstileToken || null,
          turnstileTokenExpires: sessionObj.turnstileTokenExpires || null
        }
      });
    });
  } catch (error) {
...
function createSessionMiddleware() {
  ...
  logger.info('Session configuration', {
    ...
    store_encrypted: !!sessionStoreOptions.secret
  });
