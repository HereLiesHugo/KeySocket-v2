...
/**
 * Handle Turnstile token validation and session binding
 * @param {string} tsToken - Turnstile token to validate
 * @param {string} remoteIp - Remote IP address
 * @param {Object} sessionData - Session data
 * @param {Function} done - WebSocket verification callback
 * @returns {boolean} True if validation passed, false otherwise
 */
function handleTurnstileValidation(tsToken, remoteIp, sessionData, done) {
  ...
  try {
    ...
    logger.debug('Failed to bind turnstile token to session', { 
      session_id: sessionData.sessionId,
      error: error.message 
    });
  }
...
/**
 * Extract Turnstile token from WebSocket upgrade request
...
function extractTurnstileToken(req) {
  ...
  try {
    ...
    if (req.headers['sec-websocket-protocol']) {
      tsToken = req.headers['sec-websocket-protocol'].split(',')[0].trim();
    }
    if (!tsToken && req.headers?.authorization) {
