 ...
 */
function isOriginAllowed(origin, ip) {
  // If no allowed origins configured, accept all (for backwards compatibility)
  if (!config.ALLOWED_ORIGINS || config.ALLOWED_ORIGINS.length === 0) {
    logger.debug('No allowed origins configured, accepting all origins', { ip });
    return true;
  }

  // If no origin header provided, reject
  if (!origin) {
    logger.warn('WebSocket upgrade rejected: missing Origin header', { ip });
    return false;
  }
  ...
  const isAllowed = config.ALLOWED_ORIGINS.some(allowedOrigin => {
    try {
      ...
      logger.debug('Failed to parse origin URL for comparison', {
        origin,
  ...
  if (!isAllowed) {
    logger.warn('WebSocket upgrade rejected: origin not in allow-list', {
      origin,
  ...
  const wss = new WebSocketServer({
    ...
    verifyClient: (info, done) => {
      ...
      parseWebSocketSession(info.req, async (err, sessionData) => {
        ...
        // Validate Origin header against allow-list
        const origin = info.req.headers.origin;
        if (!isOriginAllowed(origin, remoteIp)) {
          logger.warn('WebSocket upgrade rejected: origin not allowed', {
            ip: remoteIp,
            origin,
        ...
        let tsToken = null;
        try {
          // Support passing it via Sec-WebSocket-Protocol or Authorization header
          if (info.req.headers['sec-websocket-protocol']) {
            tsToken = info.req.headers['sec-websocket-protocol'].split(',')[0].trim();
          }
          ...
          logger.debug('Failed to parse WebSocket token', { error: error.message });
        }

        // If a token is provided, consume and bind it to this session
        if (tsToken) {
          if (!consumeVerifiedToken(tsToken, remoteIp)) {
            logger.warn('WebSocket upgrade rejected: invalid/expired turnstile token', {
              ip: remoteIp,
              user: sessionData.user.email
            });
          ...
          // Persist binding to session (best-effort)
          try {
            ...
            if (sessionData.sessionId && sessionStore && typeof sessionStore.set === 'function') {
              sessionStore.set(sessionData.sessionId, sess, (err) => {
                if (err) {
                  logger.warn('Failed to persist turnstileVerifiedIP to session', {
