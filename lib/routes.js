...
function getCurrentGitBranch() {
  try {
    // Attempt to use absolute path for git to mitigate security risks (S4036)
    const isWin = process.platform === 'win32';
    
    // Check common paths for git executable
    ...
    let gitPath = 'git'; // Default fallback
    ...
    logger.debug('Failed to get git branch', { error: error.message });
    return 'main';
  }
}
...
function verifyWithCloudflare(token, remoteIp, attempt = 0) {
  return new Promise((resolve, reject) => {
    ...
    const handleRetry = (error, attempt) => {
      if (attempt < config.TURNSTILE_MAX_RETRIES) {
        const backoff = 200 * Math.pow(2, attempt);
        logger.warn('Turnstile request error, retrying', {
          error: error.message,
          attempt,
          backoff
        });
        setTimeout(() => verifyWithCloudflare(token, remoteIp, attempt + 1).then(resolve).catch(reject), backoff);
      } else {
        reject(error);
      }
    };
      ...
      resp.on('end', () => {
        ...
        if (!statusOk || !looksJson) {
          if (resp.statusCode >= 500) {
            const error = new Error('turnstile provider error');
            error.status = resp.statusCode;
            error.body = data;
            error.headers = resp.headers;
            return handleRetry(error, attempt);
          }
    ...
    request.on('timeout', () => {
      request.destroy();
      handleRetry(new Error('timeout'), attempt);
    });

    request.on('error', (err) => {
      handleRetry(err, attempt);
    });
...
function registerRoutes(app, passport) {
  ...
  // Get current git branch for dynamic link updating
