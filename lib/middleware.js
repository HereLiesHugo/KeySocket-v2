...
function setupSecurityMiddleware(app) {
  ...
  // CORS middleware - strict allow-list to prevent CORS injection
  app.use((req, res, next) => {
    ...
    // Find matching origin from allow-list (never reflect untrusted input)
    const matchedOrigin = allowedOrigins.find(allowed => allowed === requestOrigin);
    
    if (matchedOrigin) {
      // Use the validated origin from our allow-list, not the request header
      res.setHeader('Access-Control-Allow-Origin', matchedOrigin);
      res.setHeader('Access-Control-Allow-Credentials', 'true');
    }
    // Note: Same-origin requests (no Origin header) don't need CORS headers
